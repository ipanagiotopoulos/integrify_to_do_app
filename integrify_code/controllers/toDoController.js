...
const insertToDo = async (req, res) => {
    try {
        ...
        if (!todoItem) return res.status(500)
            .send({
                message: 'Server error',
                error: 'Something went worng, please try again later'
            })
...
const updateToDo = async (req, res) => {
    try {
        ...
        if (!toDoItem) return res.status(404).
            send({
                message: 'Item not found',
                error: 'Item with id' + data.id + ' was not found'
            })
        if (toDoItem.userId !== req.id) return res.status(403).
            send({
...
const deleteToDo = async (req, res) => {
    ...
    if (!toDoItem) return res.status(404)
        .send({
            message: 'Item not found',
            error: 'Item with id' + req.params.id + ' was not found'
        })
    if (toDoItem.userId !== req.id) return res.status(403)
        .send({
            message: 'Forbidden resourse',
